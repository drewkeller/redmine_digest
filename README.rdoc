...
This plugin sends a summary of project activity in an email. Each project is checked for activity during the specified period of time, and if there is new activity, an email is sent to each user who has opted to receive the digest.
...
Or install rufus-scheduler and add an initializer to REDMINE_ROOT/config/initializers/digest_tasks.rb, like...
    scheduler.cron '01 00 * * 1-7' do
	  ...
	  options = { :start => 1, :days => 1, :project => 'myproject' }
